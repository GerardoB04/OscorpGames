	...
	public partial class PacManGame : Form {
		...
		private void resetGame() {
			...
			if(Leaderboard.GetScores(Leaderboard.PACMAN_GAME_NAME) == new string[] { "0" }) {
				Leaderboard.SaveScore(["9999", "9999", "9999"], Leaderboard.PACMAN_GAME_NAME);
			}
		...
		private void writeLowTimes() {
			...
			Leaderboard.SaveScore([times[0].ToString(), times[1].ToString(), times[2].ToString()], Leaderboard.PACMAN_GAME_NAME);
		}
		...
		private void pacman_Closed(object sender, FormClosedEventArgs e)
		{
			timer1.Stop();
